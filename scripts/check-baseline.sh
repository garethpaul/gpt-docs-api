...
python3 - "$SYNTAX_CHECK" "$TEST_SYNTAX_CHECK" <<'PY'
import sys
from pathlib import Path
...
test_required = (
    ...
    "Python verification must leave no generated bytecode artifacts:",
)
if any(item not in checker for item in checker_required):
    raise SystemExit("Syntax checker must compile Python sources in memory.")
if "compileall" in checker:
    raise SystemExit("Syntax checker must not write bytecode through compileall.")
if any(item not in test for item in test_required):
    raise SystemExit("Syntax checker regression must cover cache output and invalid syntax.")
PY
...
required = (
    ...
    "external-directory Make gate also passed from `/tmp`",
    "No `__pycache__`, `.pyc`, or `.pyo` artifact remained",
    "Eight isolated mutations were rejected",
    "No live OpenAI, Pinecone, DynamoDB, AWS, Twilio, API Gateway, Chalice package, or deployment operation was executed",
)
if (
    statuses != ["status: completed"]
    or "## Verification Completed\n" not in plan
...
if [ -n "$generated_python_artifacts" ]; then
  printf '%s\n' "Python verification must leave no generated bytecode artifacts:" >&2
  printf '%s\n' "$generated_python_artifacts" >&2
  exit 1
fi
